...
import { IQuestionMySuffix, defaultValue } from 'app/shared/model/question-my-suffix.model';
...
const initialState = {
  loading: false,
  errorMessage: null,
  entities: [] as ReadonlyArray<IQuestionMySuffix>,
  entity: defaultValue,
  updating: false,
  totalItems: 0,
  updateSuccess: false
...
export type QuestionMySuffixState = Readonly<typeof initialState>;

// Reducer

export default (state: QuestionMySuffixState = initialState, action): QuestionMySuffixState => {
  switch (action.type) {
...
export const getEntities: ICrudGetAllAction<IQuestionMySuffix> = (page, size, sort) => {
  const requestUrl = `${apiUrl}${sort ? `?page=${page}&size=${size}&sort=${sort}` : ''}`;
  return {
    type: ACTION_TYPES.FETCH_QUESTION_LIST,
    payload: axios.get<IQuestionMySuffix>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};

export const getEntity: ICrudGetAction<IQuestionMySuffix> = id => {
  const requestUrl = `${apiUrl}/${id}`;
  return {
    type: ACTION_TYPES.FETCH_QUESTION,
    payload: axios.get<IQuestionMySuffix>(requestUrl)
  };
};

export const createEntity: ICrudPutAction<IQuestionMySuffix> = entity => async dispatch => {
...
export const updateEntity: ICrudPutAction<IQuestionMySuffix> = entity => async dispatch => {
  const result = await dispatch({
...
export const deleteEntity: ICrudDeleteAction<IQuestionMySuffix> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
